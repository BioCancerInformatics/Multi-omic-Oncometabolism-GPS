...
## Authors: Juan Carlo S Silva, Emanuell Rodrigues de Souza, Leonardo Henrique da Silva, Ana Beatriz Garcia, Enrique Medina-Acosta
## Updated: 03/12/2024

### GET_XENA_DATA - MIRNA vs MSI
...
setwd("E:/Higor/Cancer_metabolism_project/14- Radar_Siganature_analysis/MSI_PanCan/miRNA_vs_MSI/")
...
genes <- tryCatch({
  readRDS("E:/Higor/Cancer_metabolism_project/13- Building_metabolic_signatures/Signatures_by_omic_layer/miRNA_signature.rds")
}, error = function(e) {
...
# Função para processar assinaturas
process_gene_name <- function(gene) {
  # Remover crases se presentes
  gene <- gsub("`", "", gene)
  
  # Verificar se é uma assinatura de múltiplos genes
  if (grepl("\\+", gene)) {
    # Adicionar crases em torno de cada elemento individual
    gene <- gsub("(\\b[A-Za-z0-9-]+\\b)", "`\\1`", gene)
    return(gene)
  } else {
    # Se for um gene único, remover parênteses
    return(gsub("\\(|\\)", "", gene))
  }
}

# Aplicar a função de processamento aos genes atuais
current_genes <- sapply(current_genes, process_gene_name)
