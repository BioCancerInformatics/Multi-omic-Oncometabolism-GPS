...
library(jsonlite)
library(rio)
setwd("F:/Higor/Metabolic_gene_analysis_05/3- Searching_for_transcript")

# Função para obter informações do gene e adicionar ao dataframe
get_gene_info <- function(gene_symbol) {
  base_url <- "https://rest.ensembl.org/"
  endpoint <- paste0("lookup/symbol/human/", gene_symbol, "?expand=1")
...
if (file.exists(rds_file)) {
  ...
  genes <- import("Target_genes.tsv")
  progresso_analise <- list(Genes = genes$Genes, combined_gene_df = data.frame())
