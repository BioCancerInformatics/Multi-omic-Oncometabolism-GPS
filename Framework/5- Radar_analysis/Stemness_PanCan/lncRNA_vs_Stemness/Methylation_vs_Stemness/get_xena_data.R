...
### GET_XENA_DATA - Methylation vs STEMNESS
...
setwd("F:/Higor/Cancer_metabolism_analysis_06/5- Radar_Analysis/Stemness_PanCan/lncRNA_vs_Stemness/Methylation_vs_Stemness/")

# Read the gene data from the file, trimming spaces
genes <- tryCatch({
  read_delim("data/Target_lncrna.tsv", delim = "\t", trim_ws = TRUE)
}, error = function(e) {
...
if (is.null(genes)) {
  ...
  genes <- genes[!duplicated(genes$Target_lncrna), ]
  
  # Sort the gene symbols alphabetically
  genes$Target_lncrna <- sort(genes$Target_lncrna)
  
  # Subset the genes column
  current_genes <- unique(genes$Target_lncrna)
