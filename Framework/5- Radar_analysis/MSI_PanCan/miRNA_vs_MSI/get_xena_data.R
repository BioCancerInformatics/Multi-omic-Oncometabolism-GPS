...
setwd("F:/Higor/Cancer_metabolism_analysis_06/5- Radar_Analysis/MSI_PanCan/miRNA_vs_MSI/")

# Read the gene data from the file, trimming spaces
genes <- tryCatch({
  read_delim("data/Target_mirna.tsv", delim = "\t", trim_ws = TRUE)
}, error = function(e) {
...
if (is.null(genes)) {
  ...
  genes <- genes[!duplicated(genes$Mature_mirna_id), ]
  
  # Sort the gene symbols alphabetically
  genes$Mature_mirna_id <- sort(genes$Mature_mirna_id)
  
  # Subset the genes column
  current_genes <- unique(genes$Mature_mirna_id)
