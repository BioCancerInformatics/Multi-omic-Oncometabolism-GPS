### UCSCXenaShiny Pancan omics-phenotype correlations
...
setwd("F:/Higor/Cancer_metabolism_analysis_06/5- Radar_Analysis/MSI_PanCan/Protein_vs_MSI/")
...
msi_empty_gene_names <- names(msi_gene_data)[sapply(msi_gene_data, function(x) all(is.na(x)))]

# Check if any empty gene names were found
if (length(msi_empty_gene_names) > 0) {
  cat("Empty gene names found:", msi_empty_gene_names, "\n")
  # Save the empty gene names to a .txt file in the working directory
  writeLines(as.character(msi_empty_gene_names), "int/msi_empty_gene_names.txt")
} else {
  cat("No empty gene names found.\n")
}

msi_empty_gene_names = names(msi_gene_data[which(sapply(msi_gene_data, is.null))])
if (length(msi_empty_gene_names) > 0){
...
write.table(msi_empty_gene_names,
...
for (i in 1:nrow(network_table)) {
  ...
  tryCatch({
    ...
    if (!is.null(Results_expression[["data"]])) {
      ...
      tpm_data <- Results_expression[["data"]][["tpm"]]
...
for (cutoff in correlation_cutoffs) {
  ...
  if (nrow(network_table_filtered) > 0) {
    ...
    tryCatch({
      ...
      ###### Get genes signatures for each cancer type
      
      signature_directions = sign_verification(network_table_filtered)
      
      for(i in signature_directions){
        nodes = format_gene_signatures(nodes,
                                       network_table_filtered,
                                       correlation_status = i)
      }
      
      nodes[is.na(nodes)] = ''
      
      # Salve os resultados com o nome de arquivo apropriado
      network_filename <- paste0('results/gene_cancer_msi_correlation_network_', cutoff, '.tsv')
      nodes_filename <- paste0('results/gene_cancer_msi_nodes_network_', cutoff, '.tsv')
      ...
      completion_message <- paste("Processing for msi_correlation cutoff:", cutoff, "completed.")
      ...
      no_result_message <- paste("No results for msi_correlation cutoff:", cutoff)
...
files <- list.files(path = results_directory,
                    pattern = "gene_cancer_msi_correlation_network_\\d+\\.\\d+\\.tsv",
                    full.names = TRUE)

# Process to find the highest numerical suffix
if (length(files) > 0) {
  suffixes <- gsub(".*gene_cancer_msi_correlation_network_", "", files)
  suffixes <- gsub("\\.tsv", "", suffixes)
  numeric_suffixes <- as.numeric(suffixes)
  
  if (length(numeric_suffixes) > 0) {
    highest_msi_suffix <- max(numeric_suffixes, na.rm = TRUE)
    highest_serial_file <- files[which(numeric_suffixes == highest_msi_suffix)]
    
    # Read the file with the highest serial number
    highest_data <- read.delim(highest_serial_file, stringsAsFactors = FALSE)
    highest_msi_suffix_found <- highest_msi_suffix
  }
}

# Optional: Write the highest suffix found to a text file and print it
highest_msi_suffix_found_path <- file.path(results_directory, "highest_msi_suffix_found.txt")
write.table(highest_msi_suffix_found,  # assuming 'highest_msi_suffix_found' is your data
            highest_msi_suffix_found_path,
            col.names = FALSE,
            row.names = FALSE,
            quote = FALSE)

# Optional: Print the highest suffix found
cat("The highest msi_suffix found is: ", highest_msi_suffix_found, "\n")
