...
setwd("E:/Higor/Cancer_metabolism_project/18- Tumor_cell_infiltration_signature_analysis/Transcript/")

# Importando o arquivo
Table <- import("/Higor/Cancer_metabolism_project/17- Survival_signature_analysis//Transcript/Survival_analysis_03.tsv")
Table <- Table[1:15000, ]
...
til_filtered_pvalue <- til_results %>%
  filter(p.value <= 0.05)

# Filtrando os dados para incluir apenas linhas onde o FDR seja menor ou igual a 0.05
til_filtered_padj <- til_results %>%
  filter(padj <= 0.05)
...
data <- merge(
  Table, 
  til_filtered_pvalue %>% select(cancer, gene, immune_cells, cor, padj), 
...
data02 <- merge(
  Table, 
  til_filtered_padj %>% select(cancer, gene, immune_cells, cor, padj), 
  by.x = c("cancer_types", "genes"), 
  by.y = c("cancer", "gene"), 
  all.x = TRUE
)
...
export(til_results, "TIL_results_01.tsv")

# Exportando os resultados gerais para um arquivo
export(data, "TIL_filtered_results_v1_01.tsv")

# Exportando os resultados gerais para um arquivo
export(data02, "TIL_filtered_results_v2_01.tsv")
