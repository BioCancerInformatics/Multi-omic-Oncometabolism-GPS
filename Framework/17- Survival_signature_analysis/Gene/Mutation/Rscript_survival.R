...
# Set directory
setwd("E:/Higor/Cancer_metabolism_project/17- Survival_signature_analysis/Gene/Mutation/")

# Import table
table <- import("E:/Higor/Cancer_metabolism_project/16- Cox_signature_analysis/Gene/Mutation/Cox_results.tsv")
...
group_surv_multiple <- tryCatch({
  plot_surv[["plot"]][["data"]] %>%
    ...
    summarize(
      ...
      time_80 = min(time[surv <= 0.8], na.rm = TRUE),
...
worst_prognosis_group <- if (!is.null(group_surv_multiple)) {
  if (p_val >= 0.05) {
    ...
    # Contagem de quantas vezes "WT" atinge o menor tempo em cada ponto de sobrevivência
    WT_wins <- sum(
      !is.na(group_surv_multiple$time_40) && group_surv_multiple$time_40[group_surv_multiple$group == "WT"] < group_surv_multiple$time_40[group_surv_multiple$group == "MT"],
      !is.na(group_surv_multiple$time_50) && group_surv_multiple$time_50[group_surv_multiple$group == "WT"] < group_surv_multiple$time_50[group_surv_multiple$group == "MT"],
      !is.na(group_surv_multiple$time_60) && group_surv_multiple$time_60[group_surv_multiple$group == "WT"] < group_surv_multiple$time_60[group_surv_multiple$group == "MT"],
      !is.na(group_surv_multiple$time_70) && group_surv_multiple$time_70[group_surv_multiple$group == "WT"] < group_surv_multiple$time_70[group_surv_multiple$group == "MT"],
      !is.na(group_surv_multiple$time_80) && group_surv_multiple$time_80[group_surv_multiple$group == "WT"] < group_surv_multiple$time_80[group_surv_multiple$group == "MT"],
      !is.na(group_surv_multiple$time_90) && group_surv_multiple$time_90[group_surv_multiple$group == "WT"] < group_surv_multiple$time_90[group_surv_multiple$group == "MT"]  # Added 0.9 comparison
    )
    
    MT_wins <- sum(
      !is.na(group_surv_multiple$time_40) && group_surv_multiple$time_40[group_surv_multiple$group == "MT"] < group_surv_multiple$time_40[group_surv_multiple$group == "WT"],
      !is.na(group_surv_multiple$time_50) && group_surv_multiple$time_50[group_surv_multiple$group == "MT"] < group_surv_multiple$time_50[group_surv_multiple$group == "WT"],
      !is.na(group_surv_multiple$time_60) && group_surv_multiple$time_60[group_surv_multiple$group == "MT"] < group_surv_multiple$time_60[group_surv_multiple$group == "WT"],
      !is.na(group_surv_multiple$time_70) && group_surv_multiple$time_70[group_surv_multiple$group == "MT"] < group_surv_multiple$time_70[group_surv_multiple$group == "WT"],
      !is.na(group_surv_multiple$time_80) && group_surv_multiple$time_80[group_surv_multiple$group == "MT"] < group_surv_multiple$time_80[group_surv_multiple$group == "WT"],
      !is.na(group_surv_multiple$time_90) && group_surv_multiple$time_90[group_surv_multiple$group == "MT"] < group_surv_multiple$time_90[group_surv_multiple$group == "WT"]  # Added 0.9 comparison
    )
    
    if (WT_wins > MT_wins) {
      "WT"
    } else if (MT_wins > WT_wins) {
      "MT"
    } else {
      # Critério de desempate no ponto 0.5
      ifelse(group_surv_multiple$time_50[group_surv_multiple$group == "WT"] < group_surv_multiple$time_50[group_surv_multiple$group == "MT"],
             "WT", "MT")
    }
  }
} else {
